...
!-----------------------------------------------
      Integer :: i, ifixcm, mass, L, ibeta
      Real(wp) :: v0prot_vdi, v0neut_vdi, rho0pr_vdi, v0prot_dddi, v0neut_dddi, rho0pr_dddi
      Real(wp) :: v0prot, v0neut, rho0pr, amu, amc2
      Logical :: exists
      Character(8) :: force_type, tipo
      Character(len=80) :: directname, a1_c, a2_c, z1_c, z2_c, ecm_c, L_c, filename, it_c, beta_c
...
50    Continue
!
!     Set L to zero to avoid filename issues
      L = 0
      If(imode == 1) Then
         Read(5,*) ifixb
...
!        impact parameter is always positive (right ion up from axis)
         ...
         amu = fmass(1)*fmass(2)/(fmass(1)+fmass(2))
         xb = L/sqrt(2.0_wp*amu*amc2*ecm/(hbc**2))
...
! setup the directory name for output files
      ...
      &TRIM(force_type)//"_Ecm"//TRIM(ecm_c)//"_beta"//TRIM(beta_c)//"_L"//TRIM(L_c)
